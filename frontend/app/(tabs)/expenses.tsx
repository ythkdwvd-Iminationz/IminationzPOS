...
const EXPENSE_DRAFT_KEY = "iminationz:expenses:draft:v1";
...
export default function ExpensesScreen() {
  ...
  const expenseDraft = useFormDraft<ExpenseDraft>(EXPENSE_DRAFT_KEY);
  const [expenseDraftHydrated, setExpenseDraftHydrated] = useState(false);
  const [restoreSeed, setRestoreSeed] = useState<ExpenseDraft | null>(null);

  // ---- Restore an in-progress expense batch once on mount ----
  useEffect(() => {
    if (expenseDraftHydrated || expenseDraft.restoring) return;
    const d = expenseDraft.restoredDraft;
    if (d && (d.parentName.trim() || d.childItems.length > 0)) {
      setRestoreSeed(d);
      setFormModalConfig({ open: true, targetExpense: d.targetExpense });
    }
    setExpenseDraftHydrated(true);
  }, [expenseDraft.restoring, expenseDraft.restoredDraft, expenseDraftHydrated]);

  const openNewBatch = () => {
    setRestoreSeed(null);
    setFormModalConfig({ open: true });
  };

  const openEditBatch = (target: Expense) => {
    setRestoreSeed(null);
    setDetail(null);
  ...
  return (
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <Pressable testID="add-expense-fab" onPress={openNewBatch} style={styles.fab}>
      ...
      {formModalConfig.open && (
        <ExpenseFormBatchModal
          ...
          initialDraft={restoreSeed}
          onDraftChange={expenseDraft.scheduleSave}
          onDraftClear={expenseDraft.clearDraft}
          onClose={() => { setFormModalConfig({ open: false }); expenseDraft.clearDraft(); }}
...
function ExpenseFormBatchModal({
  ...
  onDraftChange: (d: ExpenseDraft) => void;
  onDraftClear: () => void;
  onClose: () => void;
  onSaved: () => void;
}) {
  ...
  // ---- Seed fields once: from a restored draft, an existing expense
  // being edited, or blank defaults for a brand-new batch. ----
  useEffect(() => {
    if (initialDraft) {
      setParentName(initialDraft.parentName);
      setDate(initialDraft.date);
      setChildItems(initialDraft.childItems);
      setCAmount(initialDraft.cAmount);
      setCSource(initialDraft.cSource);
      setCPersonalAmt(initialDraft.cPersonalAmt);
      setCBusinessAmt(initialDraft.cBusinessAmt);
      setCNote(initialDraft.cNote);
    } else if (isEditing && targetExpense) {
  ...
  // ---- Auto-save the in-progress batch so it survives an interruption ----
  useEffect(() => {
    if (!hydrated) return;
    onDraftChange({
      targetExpense,
  ...
  }, [hydrated, targetExpense, parentName, date, childItems, cAmount, cSource, cPersonalAmt, cBusinessAmt, cNote]);
  ...
  const handleAddChild = () => {
    const amt = parseInt(cAmount, 10) || 0;
    if (amt <= 0) return;
    
    let pAmt = cSource === "personal" ? amt : cSource === "both" ? parseInt(cPersonalAmt, 10) || 0 : 0;
    let bAmt = cSource === "business" ? amt : cSource === "both" ? parseInt(cBusinessAmt, 10) || 0 : 0;

    if (cSource === "both" && pAmt + bAmt !== amt) {
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : "height"} style={styles.modalWrap}>
        <View style={[styles.sheet, { maxHeight: "95%" }]}>
          ...
          <View style={styles.sheetHeader}>
            ...
            <Pressable testID="close-expense-modal" onPress={handleClose} style={styles.closeBtn}>
          ...
          <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ paddingBottom: 40 }}>
            ...
            <View style={{ padding: 12, borderWidth: 1, borderColor: theme.color.border, borderRadius: 8, marginTop: 10 }}>
              ...
              <TextInput testID="expense-child-amount" value={cAmount} onChangeText={(v) => setCAmount(v.replace(/[^0-9]/g, ""))} keyboardType="number-pad" placeholder="Amount (₹)" placeholderTextColor={theme.color.onSurfaceTertiary} style={[styles.input, { marginBottom: 8 }]} />
              <TextInput testID="expense-child-note" value={cNote} onChangeText={setCNote} placeholder="Item Details (e.g., Gold chains, cord set bags)" placeholderTextColor={theme.color.onSurfaceTertiary} style={[styles.input, { marginBottom: 8 }]} />

              <View style={[styles.segRow, { marginBottom: 8 }]}>
                {(["personal", "business", "both"] as Source[]).map((s) => (
                  <Pressable key={s} testID={`expense-source-${s}`} onPress={() => setCSource(s)} style={[styles.seg, { borderColor: s === cSource ? theme.color.brandPrimary : theme.color.border }, s === cSource && { backgroundColor: theme.color.brandTertiary }]}>
              ...
              {cSource === "both" && (
                <View style={{ flexDirection: "row", gap: 8, marginBottom: 8 }}>
                  <TextInput testID="expense-personal-amount" value={cPersonalAmt} onChangeText={(v) => setCPersonalAmt(v.replace(/[^0-9]/g, ""))} keyboardType="number-pad" placeholder="Pers. ₹" placeholderTextColor={theme.color.onSurfaceTertiary} style={[styles.input, { flex: 1 }]} />
                  <TextInput testID="expense-business-amount" value={cBusinessAmt} onChangeText={(v) => setCBusinessAmt(v.replace(/[^0-9]/g, ""))} keyboardType="number-pad" placeholder="Bus. ₹" placeholderTextColor={theme.color.onSurfaceTertiary} style={[styles.input, { flex: 1 }]} />
                </View>
              )}

              <Pressable testID="expense-stage-item" onPress={handleAddChild} style={{ backgroundColor: theme.color.surfaceTertiary, padding: 10, borderRadius: 6, alignItems: "center" }}>
...
function SetFundModal({ visible, current, onClose, onSaved }: { visible: boolean; current: number; onClose: () => void; onSaved: () => void }) {
  ...
  const onSave = async () => {
    ...
    try { await expensesApi.setPersonalFund(parseInt(val, 10) || 0); onSaved(); } catch (e: any) { setErr(e.message || "Failed saving"); } finally { setSaving(false); }
  };
    ...
    <Modal transparent visible={visible} animationType="fade" onRequestClose={onClose}>
      <View style={styles.confirmOverlay}>
        <View style={[styles.confirmBox, { width: "88%" }]}>
          ...
          <TextInput testID="personal-fund-input" value={val} onChangeText={(v) => setVal(v.replace(/[^0-9]/g, ""))} keyboardType="number-pad" style={[styles.input, { marginTop: 12 }]} />
          {err && <Text style={styles.errText}>{err}</Text>}
