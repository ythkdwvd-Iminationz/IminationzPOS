...
function buildBillHtml(bill: Bill, exchangeHistory: ExchangeHistoryEntry[]) {
  ...
  const itemRows = bill.items
    .map(
      (i) => `
        <tr>
          <td class="td name">${escHtml(i.item_name)}${i.is_custom_price ? ' <span class="custom-tag">Custom</span>' : ""}</td>
    ...
    <head>
      ...
      <style>
        ...
        * { box-sizing: border-box; }
        ...
        .custom-tag { font-size: 9px; font-weight: 700; color: #9B111E; border: 1px solid #9B111E; border-radius: 3px; padding: 0 3px; margin-left: 3px; }
      </style>
    </head>
    <body>
      <div class="paper">
...
async function sharePdfOnWeb(
  ...
  bill.items.forEach((it) => {
    ...
    doc.setFontSize(8);
    const rawName = asciiSafe(it.item_name) + (it.is_custom_price ? " (Custom)" : "");
          ...
          <View style={styles.receipt} testID="invoice-receipt">
            ...
            {bill.items.map((it, idx) => (
              <View key={idx} style={styles.tr} testID={`invoice-line-${idx}`}>
                <View style={{ flex: 2, flexDirection: "row", alignItems: "center", flexWrap: "wrap" }}>
                  <Text style={styles.td}>{it.item_name}</Text>
                  {it.is_custom_price && (
                    <View testID={`invoice-custom-tag-${idx}`} style={styles.customTag}>
...
const styles = StyleSheet.create({
  ...
  customTagText: { color: "#9B111E", fontSize: 9, fontWeight: "700" },
  thanks: {
    textAlign: "center",
    color: RECEIPT_INK,
    fontSize: 15,
