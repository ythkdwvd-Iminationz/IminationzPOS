...
--                   bills_bill_number_key" on submitting new bills
-- ---------------------------------------------------------------------
-- Root cause:
--   public.next_bill_number(date) computed the next serial from
--   count(*) over that day's existing bills. This breaks when:
--     1. A bill from today has been deleted → count drops back and the
--        next new bill reuses an existing bill_number.
--     2. Two bills are submitted concurrently → both read the same
--        count and both try to insert the same BILL-YYYYMMDD-NNN.
--
-- Fix:
--   1. next_bill_number() now derives the next serial from the MAX
--      numeric suffix already used for that date (deletes stay skipped).
--   2. create_bill() wraps the number-generation + INSERT in a retry
--      loop that catches unique_violation and bumps the serial until
--      it succeeds — handles concurrent inserts safely.
--
-- Depends on: schema.sql, whole_numbers.sql, custom_pricing.sql having
-- been run already (this migration keeps the same create_bill signature
-- and behavior from custom_pricing.sql — including owner custom pricing,
-- whole-number rounding, and configurable discount).
--
-- Safe to re-run.
-- =====================================================================
...
-- ---------------------------------------------------------------------
-- 1. next_bill_number: MAX(numeric suffix) + 1 (delete-safe)
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  SELECT COALESCE(
           MAX(
             NULLIF(
               regexp_replace(substring(bill_number FROM char_length(v_prefix) + 1), '\D', '', 'g'),
               ''
             )::integer
           ),
           0
...
-- ---------------------------------------------------------------------
-- 2. create_bill: wrap the bill-number+insert in a retry loop that
--    catches unique_violation and re-derives the next number.
--    (Full body kept in sync with custom_pricing.sql / whole_numbers.sql)
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  -- Owner detection (custom pricing is owner-only, resolved server-side).
  BEGIN
  ...
  -- Configurable discount settings (falls back to defaults if table missing).
  ...
  -- Validate items + compute totals + normalize.
  ...
  -- Discount applies only to the catalog-priced (non-custom) subtotal.
  ...
  -- Insert bill header with retry-on-conflict for the bill_number.
  -- Bumps the serial and retries up to 25 times before giving up.
  LOOP
    ...
    EXCEPTION WHEN unique_violation THEN
      ...
      IF v_retries > 25 THEN
        RAISE EXCEPTION 'Could not allocate a unique bill_number after % retries', v_retries;
      END IF;
